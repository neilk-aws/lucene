...
/**
 * BM25FQueryParser is a query parser that uses {@link CombinedFieldQuery} for BM25F multi-field
 * scoring of term queries.
 *
 * <p>This parser is structurally similar to {@link
 * org.apache.lucene.queryparser.simple.SimpleQueryParser} but produces {@link CombinedFieldQuery}
 * instances for default (term) queries instead of per-field {@code TermQuery} combined with {@code
 * BooleanQuery}. This enables true BM25F scoring across multiple fields.
 *
 * <p>Phrase, prefix, and fuzzy queries still operate on a per-field basis, since {@link
 * CombinedFieldQuery} only supports single-term queries.
 *
 * <p>The query syntax is identical to {@link
...
public class BM25FQueryParser extends QueryBuilder {
  ...
  public BM25FQueryParser(Analyzer analyzer, Map<String, Float> weights, int flags) {
    ...
    this.flags = flags;
    // validate all weights >= 1.0f as required by CombinedFieldQuery
    for (Map.Entry<String, Float> e : weights.entrySet()) {
      if (e.getValue() < 1f) {
        throw new IllegalArgumentException(
            "weight must be >= 1, got " + e.getValue() + " for field " + e.getKey());
      }
    }
  }
  ...
  /**
   * Factory method to generate a standard query (no phrase or prefix operators). Uses {@link
   * CombinedFieldQuery} for BM25F scoring across all fields.
   */
  protected Query newDefaultQuery(String text) {
    CombinedFieldQuery.Builder b = new CombinedFieldQuery.Builder(text);
    for (Map.Entry<String, Float> e : weights.entrySet()) {
      b.addField(e.getKey(), e.getValue());
    }
    return b.build();
  }
